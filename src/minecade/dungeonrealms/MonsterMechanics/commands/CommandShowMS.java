...
public class CommandShowMS implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length != 1) {
			p.sendMessage(
					ChatColor.RED + "" + ChatColor.BOLD + "Incorrect Syntax. " + ChatColor.RED + "/showms <radius>");
			return true;
		}
			...
			for (j = -radius; j <= radius; j++) {
				for (k = -radius; k <= radius; k++) {
					...
					if (MonsterMechanics.mob_spawns.containsKey(loc)) {
						...
						m.addRunCommand(
								ChatColor.GRAY + "Spawner: X:" + loc.getBlockX() + " Y:" + loc.getBlockY() + " Z:"
										+ loc.getBlockZ(),
								ChatColor.GRAY, "/drtppos " + (0.5D + loc.getBlockX()) + " " + (0.5D + loc.getBlockY())
										+ " " + (0.5D + loc.getBlockZ()));
						m.sendToPlayer(p);
						// }
					}
