	...
	public void run() {
		while (true) {
			...
			for (EntityTargetEvent e : MonsterMechanics.async_entity_target) {
				...
				if (!(PetMechanics.inv_pet_map.containsKey(e.getEntity()))
						&& !MonsterMechanics.mob_target.containsKey(e.getEntity())
						&& !(MonsterMechanics.last_mob_message_get.containsKey(p.getName()))) {
					...
					if (do_i_speak <= 12) {
						...
						if (le.hasMetadata("mobname")) {
							if (message.contains(":")) {
								// Bandit: Hey noob.
								MonsterMechanics.async_message_send.put(p,
										ChatColor.RED + le.getMetadata("mobname").get(0).asString() + ":"
												+ ChatColor.WHITE
				...
				if (!MonsterMechanics.approaching_mage_list.contains(ent) && (weapon.getType() == Material.WOOD_HOE
						|| weapon.getType() == Material.STONE_HOE || weapon.getType() == Material.IRON_HOE
						|| weapon.getType() == Material.DIAMOND_HOE || weapon.getType() == Material.GOLD_HOE)) {
					/*
					 * Projectile pj = null;
					 * if(ItemMechanics.getItemTier((weapon)) == 1){ pj =
					 * le.launchProjectile(Snowball.class); }
					 * if(ItemMechanics.getItemTier((weapon)) == 2){ pj =
					 * le.launchProjectile(SmallFireball.class); }
					 * if(ItemMechanics.getItemTier((weapon)) == 3){ pj =
					 * le.launchProjectile(EnderPearl.class);
					 * pj.setVelocity(pj.getVelocity().multiply(3)); }
					 * if(ItemMechanics.getItemTier((weapon)) == 4){ pj =
					 * le.launchProjectile(WitherSkull.class); }
					 * if(ItemMechanics.getItemTier((weapon)) == 5){ pj =
					 * le.launchProjectile(LargeFireball.class); }
					 * 
					...
					le.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, Integer.MAX_VALUE, 2)); // Make
																										// them
																										// super
																										// slow
																										// so
