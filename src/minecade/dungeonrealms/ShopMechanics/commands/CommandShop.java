			...
			if (args[0].equalsIgnoreCase("info")) {
				if (ShopMechanics.doesPlayerHaveShopSQL(op.getUniqueId())) {
					...
					if (local_server_num != server_num) {
						...
						p.sendMessage(ChatColor.GRAY + "Type /shop info " + args[1]
								+ " on that server for more information.");
					} else {
						p.sendMessage(
								ChatColor.RED + args[1] + " has an unnamed shop with nothing in it on this server.");
					}
				} else {
					...
					if (ShopMechanics.doesPlayerHaveShopSQL(op.getUniqueId())) {
						...
						if (local_server_num != server_num) {
							...
							p.sendMessage(ChatColor.RED + args[1]
									+ " has an unnamed shop that is empty.  It cannot be opened.");
						}
					} else {
				...
				if (!ShopMechanics.hasLocalShop(op.getUniqueId()) || !ShopMechanics.inverse_shop_owners.containsKey(op.getUniqueId())) {
					...
					if (ShopMechanics.doesPlayerHaveShopSQL(op.getUniqueId())) {
						...
						if (local_server_num != server_num) {
							...
							p.sendMessage(ChatColor.RED + args[1]
									+ " has an unnamed shop that is empty on this server.  It is already closed.");
						}
					} else {
						p.sendMessage(ChatColor.RED + " Player " + args[1]
								+ " does not have a shop on any server. (This command is case sensitive!)");
				...
				if (!ShopMechanics.hasLocalShop(op.getUniqueId()) || !ShopMechanics.inverse_shop_owners.containsKey(op.getUniqueId())) {
					...
					if (ShopMechanics.doesPlayerHaveShopSQL(op.getUniqueId())) {
						...
						if (local_server_num != server_num) {
							...
							p.sendMessage(
									ChatColor.RED + args[1] + " has an unnamed shop on this server that is empty.");
						}
					} else {
						p.sendMessage(ChatColor.RED + "Player " + args[1]
								+ " does not have a shop on any server. (This command is case sensitive!)");
