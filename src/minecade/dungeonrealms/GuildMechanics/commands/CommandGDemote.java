...
public class CommandGDemote implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (!(GuildMechanics.areGuildies(p.getName(), p_name_2demote))) {
			p.sendMessage(
					ChatColor.RED + "" + ChatColor.BOLD + p_name_2demote + ChatColor.RED + " is not in YOUR guild.");
			return true;
		}

		if (!GuildMechanics.isGuildOfficer(p_name_2demote) && !GuildMechanics.isGuildCoOwner(p_name_2demote)) {
			p.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + p_name_2demote + ChatColor.RED + " is not yet a "
					+ ChatColor.UNDERLINE + "guild officer");
			p.sendMessage(ChatColor.GRAY + "Use " + ChatColor.RED + "/gpromote " + p_name_2demote + ChatColor.GRAY
