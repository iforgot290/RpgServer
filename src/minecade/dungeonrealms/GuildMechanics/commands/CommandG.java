	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length == 0) {
			if(!(GuildMechanics.guild_only.contains(p.getUniqueId()))) {
				...
				p.sendMessage(ChatColor.DARK_AQUA + "Messages will now be default sent to <" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(GuildMechanics.getGuild(p.getUniqueId())) + ChatColor.DARK_AQUA + ">. Type " + ChatColor.UNDERLINE + "/l <msg>" + ChatColor.DARK_AQUA + " to speak in local.");
