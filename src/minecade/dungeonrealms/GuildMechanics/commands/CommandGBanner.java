...
public class CommandGBanner implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (!(GuildMechanics.isGuildLeader(p.getName()) || GuildMechanics.isGuildCoOwner(p.getName()))) {
			p.sendMessage(ChatColor.RED + "You must be the " + ChatColor.BOLD + "GUILD LEADER" + ChatColor.RED
					+ " to use " + ChatColor.BOLD + "/gbanner.");
			return true;
		}
		...
		if (args.length != 1) {
			...
			p.sendMessage(ChatColor.GRAY
					+ "Upload your guild banner to imgur.com, then paste the 'direct link' as the command argument.");
		...
		if (args.length == 1) {
			...
			if (img_code.contains(".")) {
				img_code = img_code.substring(0, img_code.lastIndexOf(".")); // Gets
																				// rid
																				// of
																				// the
																				// file
			...
			p.sendMessage(ChatColor.GREEN + "Your Guild Banner has been SET. (" + ChatColor.UNDERLINE + better_url
					+ ChatColor.GREEN + ")");
