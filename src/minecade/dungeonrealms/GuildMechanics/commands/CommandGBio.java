		...
		if (args.length == 0) {
			...
			if (!(GuildMechanics.isGuildLeader(p.getName()) || GuildMechanics.isGuildCoOwner(p.getName()))) {
				...
				if ((!(GuildMechanics.guild_bio.containsKey(GuildMechanics.getGuild(p))))
					...
					// NO MOTD.
					p.sendMessage(ChatColor.RED + "No " + ChatColor.BOLD + "GUILD BIOGRAPHY" + ChatColor.RED
							+ " currently set.");
				...
				if (GuildMechanics.guild_bio.containsKey(g_name) && GuildMechanics.guild_bio.get(g_name) != null) {
					...
					p.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD
							+ GuildMechanics.guild_handle_map.get(g_name) + ChatColor.DARK_AQUA + "> " + ChatColor.BOLD
							+ "Guild Biography: " + ChatColor.DARK_AQUA + bio);
				}
				return true;
			}
			...
			if (GuildMechanics.guild_bio_dynamic.containsKey(p.getName())) {
				p.sendMessage(ChatColor.RED
						+ "You already have a pending guild biography. Type 'cancel' to void that one before starting another.");
				return true;
			}
			...
			p.sendMessage(ChatColor.GRAY + "Send your typed message at any time as a line break. Type '"
					+ ChatColor.GREEN.toString() + "confirm" + ChatColor.GRAY + "' when you're done OR '"
					+ ChatColor.RED + "cancel" + ChatColor.GRAY + "' to void this process.");
			p.sendMessage("");
