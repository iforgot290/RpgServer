...
public class CommandGKick implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (g_rank < 2) { // 2 = officer, 3 = co-owner, 4 = owner, -> 1 is just
							// a member.
			p.sendMessage(ChatColor.RED + "You must be at least a guild " + ChatColor.BOLD + "OFFICER" + ChatColor.RED
					+ " to use " + ChatColor.BOLD + "/gkick");
			return true;
		}
		...
		if (op.getPlayer() != null) {
			...
			pl.sendMessage(ChatColor.RED + "You have been " + ChatColor.BOLD + "KICKED" + ChatColor.RED
					+ " out of your guild.");
		...
		for (Player pl : GuildMechanics.getOnlineGuildMembers(g_name)) {
			pl.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(g_name)
					+ ChatColor.DARK_AQUA + "> " + ChatColor.DARK_AQUA + p_to_kick + " has been " + ChatColor.UNDERLINE
					+ "kicked" + ChatColor.DARK_AQUA + " by " + p.getName() + ".");
		}
