...
public class CommandShowLoot implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length != 1) {
			p.sendMessage(
					ChatColor.RED + "" + ChatColor.BOLD + "Incorrect Syntax. " + ChatColor.RED + "/showloot <radius>");
			return true;
		}
		...
		for (i = -radius; i <= radius; i++) {
			for (j = -radius; j <= radius; j++) {
				for (k = -radius; k <= radius; k++) {
					...
					if (LootMechanics.loot_spawns.containsKey(loc) || (InstanceMechanics.instance_loot
							.containsKey(loc.getWorld().getName())
