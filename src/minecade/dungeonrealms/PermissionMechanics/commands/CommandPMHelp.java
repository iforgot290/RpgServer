...
public class CommandPMHelp implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		p.sendMessage(ChatColor.WHITE + "/mute <PLAYER> <TIME(minutes)>" + ChatColor.GRAY
				+ " Mutes PLAYER for TIME minutes from local and global chat.");
		p.sendMessage(ChatColor.WHITE + "/kick <PLAYER> <REASON>" + ChatColor.GRAY
				+ " Kicks PLAYER and displays REASON to them.");
		p.sendMessage(ChatColor.WHITE + "/ban <PLAYER> <TIME(hours)>" + ChatColor.GRAY
				+ " Bans PLAYER for TIME minutes from all servers.");
		p.sendMessage(ChatColor.WHITE + "/flag <PLAYER> (REASON)" + ChatColor.GRAY
				+ " Flags PLAYER for REASON for other mods to see. Leave REASON blank to view flags on player.");
		return true;
	}
