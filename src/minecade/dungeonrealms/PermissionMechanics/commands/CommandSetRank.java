	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length != 2) {
			if (p != null) {
				...
				p.sendMessage(
						ChatColor.RED + "Ranks: " + ChatColor.GRAY + "Default | SUB | SUB+ | SUB++ | PMOD | GM | YT |");
			}
			return true;
		}
		...
		if (!(rank.equalsIgnoreCase("default")) && !(rank.equalsIgnoreCase("wd")) && !(rank.equalsIgnoreCase("sub"))
				&& !(rank.equalsIgnoreCase("sub+")) && !(rank.equalsIgnoreCase("sub++"))
				&& !(rank.equalsIgnoreCase("PMod")) && !(rank.equalsIgnoreCase("GM"))
				&& !(rank.equalsIgnoreCase("yt"))) {
			if (p != null) {
				p.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "Invalid Rank '" + rank + "'");
				p.sendMessage(ChatColor.RED + "/setrank <PLAYER> <RANK>");
				p.sendMessage(ChatColor.RED + "Ranks: " + ChatColor.GRAY
						+ "Default | SUB | SUB+ | SUB++ | PMOD | GM | WD | YT |");
			}
			return true;
		}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				CommunityMechanics.sendPacketCrossServer(
						"[rank_map]" + player.getUniqueId().toString() + ":" + rank.toLowerCase(), -1, true);
				...
				if (psender != null) {
					psender.sendMessage(ChatColor.GREEN + "You have set the user " + player.getName()
							+ " to the rank of " + rank + " on all Dungeon Realm servers.");
				}
			}
