...
public class StatsGUIWorker extends DynamicMenu {
	...
	public static void setCustomStatAllocationSlot(int slot, Player plr, ItemStack item) {
		...
		plr.sendMessage(ChatColor.GRAY + "Type the " + ChatColor.BOLD.toString() + ChatColor.GREEN + "number"
				+ ChatColor.GRAY + " of stat points you wish to allocate to your " + ChatColor.UNDERLINE.toString()
				+ ChatColor.GREEN + ChatColor.stripColor(item.getItemMeta().getDisplayName().toUpperCase())
				+ ChatColor.GRAY
