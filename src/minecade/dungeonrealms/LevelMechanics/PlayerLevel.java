...
public class PlayerLevel {
	...
	private int numWarnings; // number of free stat point warnings sent so far
								// to prevent spam
	private int allocateSlot; // when a player is allocating a custom amount of
								// points to a stat
	private int numResets; // number of stat resets the player has already had
	private boolean isResetting; // flag for if the player has talked to the
									// reset NPC
	private String resetCode; // code for stat reset
	private int resetCost; // cost to reset stats for player
	public final static int POINTS_PER_LEVEL = 6; // points per level. Change
													// this to change the global
													// value.
	public final static String FREE_STAT_NOTICE = ChatColor.GREEN + "*" + ChatColor.GRAY + "You have available "
			+ ChatColor.GREEN + "/stat" + ChatColor.GRAY + " points. Click " + ChatColor.GREEN.toString()
	...
	public void addXP(int xp) {
		...
		if (PlayerManager.getPlayerModel(p_name).getToggleList() != null
			...
			if (p == null)
				return;
			p.sendMessage(ChatColor.YELLOW + "" + ChatColor.BOLD + "          +" + ChatColor.YELLOW + (int) xp
					+ ChatColor.BOLD + " EXP" + ChatColor.YELLOW + ChatColor.GRAY + " [" + getXP() + ChatColor.BOLD
					+ "/" + ChatColor.GRAY + (int) getEXPNeeded(getLevel()) + " EXP]");
	...
	public int getEXPNeeded(int level) {
		...
		return (int) ((100 * Math.pow(level, 2)) * difficulty); // patch 1.9 exp
																// formula
	}
	...
	/**
	 * @param Set
	 *            the amount of free stat points for the user
	 */
