...
public class AceronListener implements Listener {
	...
	public void minionDeath(EntityDeathEvent e) {
		if (e.getEntity().getWorld().getName().toLowerCase().contains("onewolfedungeon")) {
						...
						for (Entry<Entity, Entity> aceron_wolves : aceron_wolf.entrySet()) {
							if (aceron_wolves.getValue().equals(e.getEntity())) {
								...
								for (Player p : e.getEntity().getWorld().getPlayers()) {
									p.sendMessage(ChatColor.GOLD.toString() + ChatColor.UNDERLINE + "Aceron the Wicked:"
											+ ChatColor.WHITE + " Noooo, I will avenge you Diner!");
									p.sendMessage(ChatColor.RED + "Aceron now has 1.5x armor!");
								}
								MonsterMechanics.mob_armor.put(e.getEntity(),
										(int) (MonsterMechanics.mob_armor.get(e.getEntity()) * 1.5));
								break;
							}
						}
					}
			...
			for (Entry<Entity, List<Entity>> minions : BossMechanics.aceron_minions.entrySet()) {
				...
				if (ents.size() <= 0) {
					for (Player p : e.getEntity().getWorld().getPlayers()) {
						p.sendMessage(ChatColor.GOLD + "" + ChatColor.UNDERLINE + "Aceron the Wicked:" + ChatColor.WHITE
		...
		if (aceron_wolf.containsValue(wolf)) {
			...
			if (should_i_crit <= 30) {
				...
				if (PlayerManager.getPlayerModel(p).getToggleList() != null
					...
					p.playSound(p.getLocation(), Sound.ENTITY_PLAYER_BURP, 1, .3F);
				}
				critted = true;
			}
			...
			// p.sendMessage(ChatColor.GOLD.toString() + ChatColor.UNDERLINE +
			// "Aceron the Wicked:" + ChatColor.WHITE +
	...
	public void onEntityDamage(EntityDamageEvent e) {
		if (e.getEntity().getWorld().getName().toLowerCase().contains("onewolfedungeon")) {
			if (BossMechanics.boss_map.containsKey(e.getEntity())
					&& BossMechanics.boss_map.get(e.getEntity()).equalsIgnoreCase("aceron")) {
				if (BossMechanics.invincible_mob.contains(e.getEntity())
						|| BossMechanics.aceron_minions.containsKey(e.getEntity())) {
					e.setCancelled(true);
					e.setDamage(0);
					return;
				}
				...
				if (should_i_retreat <= 5 && !BossMechanics.is_jumping.contains(boss) && percent_hp > .5) { // 5%
																											// chance
																											// of
																											// that
					...
					for (int i = 0; i < amount_to_spawn; i++) {
						ents.add(MonsterMechanics.spawnTierMob(boss.getLocation(),
								Math.random() >= .5D ? EntityType.SKELETON : EntityType.ZOMBIE, 4, -1,
				...
				if (percent_hp <= 30 && !BossMechanics.is_jumping.contains(boss)
					...
					Wolf wolf = (Wolf) MonsterMechanics.spawnBossMob(boss.getLocation(), EntityType.WOLF, "",
							"Diner of Bones", 4);
					...
					for (Player p : boss.getWorld().getPlayers()) {
						p.sendMessage(ChatColor.GOLD.toString() + ChatColor.UNDERLINE + "Aceron the Wicked:"
								+ ChatColor.WHITE + " Dinner is served.");
					}
