...
public class CommandGR implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length <= 0) {
			pl.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "Incorrect syntax. You must supply a message! "
					+ ChatColor.BOLD + "/gr <MESSAGE>");
			return true;
		}
		...
		if (ChatMechanics.mute_list.containsKey(pl.getUniqueId())) {
			long time_left = ChatMechanics.mute_list.get(pl.getUniqueId());
			pl.sendMessage(ChatColor.RED + "You are currently " + ChatColor.BOLD + "GLOBALLY MUTED" + ChatColor.RED
		...
		String rank = PermissionMechanics.getRank(pl.getUniqueId());
		...
		if (PlayerManager.getPlayerModel(pl).getGlobalChatDelay() != 0) {
			...
			if ((cur_time - old_time) < (personal_delay * 1000) && !(pl.isOp()) && !(rank.equalsIgnoreCase("GM"))
					&& !(rank.equalsIgnoreCase("PMOD") && !(rank.equalsIgnoreCase("WD")))) {
				int s_delay_left = personal_delay - (int) ((cur_time - old_time) / 1000);
				pl.sendMessage(ChatColor.RED + "You can send another GLOBAL MESSAGE in " + s_delay_left + ChatColor.BOLD
						+ "s");
				return true;
			}
		}
		...
		if (PlayerManager.getPlayerModel(pl).getToggleList() != null
				&& PlayerManager.getPlayerModel(pl).getToggleList().contains("guild")) {
			pl.sendMessage(ChatColor.RED + "You currently have global messaging " + ChatColor.BOLD + "DISABLED."
		...
		if (message.contains("@i@") && handitem.getType() != Material.AIR) {
			...
			normal.addItem(handitem, ChatColor.BOLD + "SHOW", ChatColor.UNDERLINE);
			...
			filter.addItem(handitem, ChatColor.BOLD + "SHOW", ChatColor.UNDERLINE);
			filter.addText(ChatMechanics.censorMessage(after));
		}

		for (Player p : Bukkit.getServer().getOnlinePlayers()) {
			if (CommunityMechanics.isPlayerOnIgnoreList(pl, p) || CommunityMechanics.isPlayerOnIgnoreList(pl, p)) {
				continue; // Either sender has the sendee ignored or vice versa,
			...
			if (PlayerManager.getPlayerModel(p).getToggleList() != null
					&& PlayerManager.getPlayerModel(p).getToggleList().contains("guildchat")) {
				continue; // They have guildchat off, so stop them from
							// receiving the messages.
			}
			if (TutorialMechanics.onTutorialIsland(p)) {
				continue; // Don't send any outside chat to players on tutorial
							// island.
			}

			if (normal != null) {
				JSONMessage toSend = normal;
				if (ChatMechanics.hasAdultFilter(p)) {
					toSend = filter;
				}
				...
				ChatColor p_color = ChatMechanics.getPlayerColor(pl, p);
				p.sendMessage(ChatColor.RED + "<" + ChatColor.BOLD + "GR" + ChatColor.RED + ">" + " " + prefix + p_color
						+ aprefix + message);
			}
		}
