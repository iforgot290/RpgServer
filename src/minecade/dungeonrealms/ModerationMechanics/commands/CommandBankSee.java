...
public class CommandBankSee implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length <= 0) {
			if (p != null) {
				p.sendMessage(
						ChatColor.RED + "" + ChatColor.BOLD + "Invalid Syntax. " + ChatColor.RED + "/banksee <PLAYER>");
			}
			return true;
		}
		...
		if (ModerationMechanics.looking_into_offline_bank.containsKey(p.getName())) {
			...
			// Otherwise they should already be loaded so let it handle the
			// code.
		}
		Inventory inv = Bukkit.createInventory(null, 54, "CLONE OF " + p_name);
		Inventory inv_clone = null;
		p_name = op.getName();
		if (MoneyMechanics.bank_contents.containsKey(op.getUniqueId())) {
			// Data is already locally downloaded, we're in luck.
			inv_clone = MoneyMechanics.bank_contents.get(op.getUniqueId()).get(0);
			...
			final boolean loaded = MoneyMechanics.downloadBankDatabaseData(op.getUniqueId());
			...
			// p.sendMessage(ChatColor.RED + "The player " + p_name + "'s bank
			// data is not loaded, and therfore cannot be displayed.");
			// p.sendMessage(ChatColor.GRAY + "In a later update, I will make it
			// possible to view offline bank data.");
		}
