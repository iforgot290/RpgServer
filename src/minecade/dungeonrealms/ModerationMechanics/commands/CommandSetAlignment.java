...
public class CommandSetAlignment implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (!(alignment.contains("evil") || alignment.contains("chaotic") || alignment.contains("neutral")
				|| alignment.contains("lawful") || alignment.contains("good"))) {
		...
		p.sendMessage(
				ChatColor.AQUA + "You have set " + args[0] + "'s alignment to: "
						+ (alignment.equalsIgnoreCase("evil") ? ChatColor.RED
								: alignment.equalsIgnoreCase("neutral") ? ChatColor.YELLOW : ChatColor.GREEN)
						+ alignment);
		Bukkit.getPlayer(args[0])
				.sendMessage(ChatColor.AQUA + p.getName() + " has set your alignment to: "
						+ (alignment.equalsIgnoreCase("evil") ? ChatColor.RED
								: alignment.equalsIgnoreCase("neutral") ? ChatColor.YELLOW : ChatColor.GREEN)
						+ alignment);
		return true;
