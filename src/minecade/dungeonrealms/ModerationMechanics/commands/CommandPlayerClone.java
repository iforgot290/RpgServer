...
public class CommandPlayerClone implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (p != null) {
			...
			Hive.sql_query.add(
					"CREATE TEMPORARY TABLE tmp_playerdb SELECT * FROM player_database WHERE p_name='" + p_name + "'");
			...
			Hive.sql_query
					.add("CREATE TEMPORARY TABLE tmp_bankdb SELECT * FROM bank_database WHERE p_name='" + p_name + "'");
			...
			Hive.sql_query
					.add("CREATE TEMPORARY TABLE tmp_shopdb SELECT * FROM shop_database WHERE p_name='" + p_name + "'");
			...
			new BukkitRunnable() {
				...
				public void run() {
					if (Bukkit.getPlayer(f_to_name) != null) {
						Bukkit.getPlayer(f_to_name)
								.kickPlayer(p_name + "'s data is copying, please wait 5-10 seconds and login.");
					}
				}
