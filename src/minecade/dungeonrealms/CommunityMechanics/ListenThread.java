...
public class ListenThread extends Thread {
	...
	public void run() {
		ServerSocket ss;
		int port = CommunityMechanics.server_list.get(Main.getCommunityMechanics().getServerNum()).contains(":")
				? Integer.parseInt(
						CommunityMechanics.server_list.get(Main.getCommunityMechanics().getServerNum()).split(":")[1])
		...
		try {
			...
			CommunityMechanics.log
					.info("[CommunityMechanics] LISTENING on port " + port + " @ " + Bukkit.getIp() + " ...");
			...
			while (true) {
				...
				if (!(CommunityMechanics.ip_whitelist.contains(ip))) {
					CommunityMechanics.log
							.info("[CommunityMechanics] Illegal connection on port " + port + " by " + ip);
					clientSocket.close();
					continue;
				}
