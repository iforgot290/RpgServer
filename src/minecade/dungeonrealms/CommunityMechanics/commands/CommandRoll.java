...
public class CommandRoll implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		for(Entity ent : p.getNearbyEntities(20, 20, 20)) {
			...
			if(player.getLocation().distanceSquared(loc) < 400 && !(CommunityMechanics.isPlayerOnIgnoreList(player, p))) {
				player.sendMessage(ChatColor.BOLD + "" + ChatColor.GRAY + ChatMechanics.getPlayerPrefix(p, true) + ChatMechanics.getPlayerColor(p, player) + p.getName() + ChatColor.GRAY + " has rolled a " + ChatColor.UNDERLINE + ChatColor.BOLD + roll + ChatColor.GRAY + " out of " + ChatColor.UNDERLINE + ChatColor.BOLD + args[0] + ".");
			}
		}
		
		p.sendMessage(ChatColor.BOLD + "" + ChatColor.GRAY + ChatMechanics.getPlayerPrefix(p, true) + ChatMechanics.getPlayerColor(p, p) + p.getName() + ChatColor.GRAY + " has rolled a " + ChatColor.UNDERLINE + ChatColor.BOLD + roll + ChatColor.GRAY + " out of " + ChatColor.UNDERLINE + ChatColor.BOLD + args[0] + ".");
		return true;
	}
