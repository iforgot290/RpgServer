...
public enum ItemType {
	...
	STAFF(Material.WOOD_HOE, "Staff", Material.STONE_HOE, "Battlestaff", Material.IRON_HOE, "Wizard Staff",
			Material.DIAMOND_HOE, "Ancient Staff", Material.GOLD_HOE, "Legendary Staff"), AXE(Material.WOOD_AXE,
					"Hatchet", Material.STONE_AXE, "Great Axe", Material.IRON_AXE, "War Axe", Material.DIAMOND_AXE,
					"Ancient Axe", Material.GOLD_AXE, "Legendary Axe"), SWORD(Material.WOOD_SWORD, "Shortsword",
							Material.STONE_SWORD, "Broadsword", Material.IRON_SWORD, "Magic Sword",
							Material.DIAMOND_SWORD, "Ancient Sword", Material.GOLD_SWORD,
							"Legendary Sword"), POLEARM(Material.WOOD_SPADE, "Spear", Material.STONE_SPADE, "Halberd",
									Material.IRON_SPADE, "Magic Polearm", Material.DIAMOND_SPADE, "Ancient Polearm",
									Material.GOLD_SPADE, "Legendary Polearm"), BOW(Material.BOW, "Shortbow",
											Material.BOW, "Longbow", Material.BOW, "Magic Bow", Material.BOW,
											"Ancient Bow", Material.BOW, "Legendary Bow"),

	HELMET(Material.LEATHER_HELMET, "Leather Coif", Material.CHAINMAIL_HELMET, "Medium Helmet", Material.IRON_HELMET,
			"Full Helmet", Material.DIAMOND_HELMET, "Ancient Full Helmet", Material.GOLD_HELMET,
			"Legendary Full Helmet"), CHESTPLATE(Material.LEATHER_CHESTPLATE, "Leather Chestplate",
					Material.CHAINMAIL_CHESTPLATE, "Chainmail", Material.IRON_CHESTPLATE, "Platemail",
					Material.DIAMOND_CHESTPLATE, "Magic Platemail", Material.GOLD_CHESTPLATE,
					"Legendary Platemail"), LEGGINGS(Material.LEATHER_LEGGINGS, "Leather Leggings",
							Material.CHAINMAIL_LEGGINGS, "Chainmail Leggings", Material.IRON_LEGGINGS,
							"Platemail Leggings", Material.DIAMOND_LEGGINGS, "Magic Platemail Leggings",
							Material.GOLD_LEGGINGS, "Legendary Platemail Leggings"), BOOTS(Material.LEATHER_BOOTS,
									"Leather Boots", Material.CHAINMAIL_BOOTS, "Chainmail Boots", Material.IRON_BOOTS,
									"Platemail Boots", Material.DIAMOND_BOOTS, "Magic Platemail Boots",
									Material.GOLD_BOOTS, "Legendary Platemail Boots");
	...
	private ItemType(Material t1, String t1Name, Material t2, String t2Name, Material t3, String t3Name, Material t4,
			String t4Name, Material t5, String t5Name) {
		this.t1 = t1;
		this.t1Name = t1Name;
	...
	public static boolean isWeapon(ItemType t) {
		if (t != STAFF && t != AXE && t != SWORD && t != POLEARM && t != BOW)
			return false;
		return true;
	}

	public static boolean isArmor(ItemType t) {
		if (t != HELMET && t != CHESTPLATE && t != LEGGINGS && t != BOOTS)
			return false;
		return true;
	}
	...
	public ItemStack generateWeapon(ItemTier tier, ItemStack original, ItemRarity rarityOverride) {
		...
		Random r = new Random();
		boolean life_steal = false, slow = false, elemental_dmg = false, knockback = false, crit_hit = false,
				pure_dmg = false, armor_pen = false,
				blind = false/* , specific_dmg = false */;
		...
		if ((dmg_range_check <= 80) || rarityOverride == ItemRarity.COMMON) { // Tier
																				// 1
																				// (low)
		...
		if ((dmg_range_check > 80 && dmg_range_check < 95) || rarityOverride == ItemRarity.UNCOMMON) { // Tier
																										// 1
																										// (med)
		...
		if ((dmg_range_check >= 95 && dmg_range_check != 99) || rarityOverride == ItemRarity.RARE) { // Tier
																										// 1
																										// (high)
		...
		if ((dmg_range_check == 99) || rarityOverride == ItemRarity.UNIQUE) { // Tier
																				// 1
																				// (high)
		...
		if (type == BOW) {
			if (slow == true) {
				...
				// Take from first index of slow= to next index of : from that
				// point.
			}
		} else {
			if (pure_dmg == true) {
			...
			if (accuracy == true) {
				...
				// Take from first index of kb= to next index of : from that
				// point.
			}
		...
		if (vs_modifier_amount > 0) {
			...
			wep_description += ChatColor.RED.toString() + "vs. " + vs_modifier_type.toUpperCase() + ": +"
					+ vs_modifier_amount + "% DMG,";
		}
		...
		if (elemental_dmg == true) {
			...
			if (blind == true || wep_name.contains("of")) {
				wep_name = wep_name + " " + element_dmg_type.substring(0, 1).toUpperCase()
						+ element_dmg_type.substring(1, element_dmg_type.length());
			} else {
				wep_name = wep_name + " of " + element_dmg_type.substring(0, 1).toUpperCase()
						+ element_dmg_type.substring(1, element_dmg_type.length());
			}

			wep_description += ChatColor.RED.toString() + element_dmg_type.toUpperCase() + " DMG: +"
					+ element_dmg_amount + ",";
	...
	public ItemStack generateArmor(ItemTier tier, ItemStack original, int moblevel, ItemRarity rarityOverride) {
		...
		Random r = new Random();
		Material m = null;
		boolean hp_regen = false, /* hp_increase = false, */energy_regen = false, block = false, dodge = false,
				thorns = false, reflection = false, gem_find = false, item_find = false;
		...
		if ((armor_range_check <= 80 && ((moblevel != -1) ? moblevel >= 1 : true))
				|| rarityOverride == ItemRarity.COMMON) {
			rarity = ChatColor.GRAY.toString() + ChatColor.ITALIC.toString() + "Common";
			if (tier == ItemTier.T1) {
				// int min_num = 1 + 1;
				// int max_num = 2 + 1;
		...
		if ((armor_range_check > 80 && armor_range_check < 95 && ((moblevel != -1) ? moblevel >= 2 : true))
				|| rarityOverride == ItemRarity.UNCOMMON) {
			rarity = ChatColor.GREEN.toString() + ChatColor.ITALIC.toString() + "Uncommon";
			if (tier == ItemTier.T1) {
				int min_num = 1 + 1;
				int max_num = 2 + 1;
		...
		if ((armor_range_check >= 95 && armor_range_check != 99 && ((moblevel != -1) ? moblevel >= 3 : true))
				|| rarityOverride == ItemRarity.RARE) {
			rarity = ChatColor.AQUA.toString() + ChatColor.ITALIC.toString() + "Rare";
			if (tier == ItemTier.T1) {
				int min_num = 1 + 1;
				int max_num = 2 + 1;
		...
		if ((armor_range_check == 99 && ((moblevel != -1) ? moblevel >= 4 : true))
				|| rarityOverride == ItemRarity.UNIQUE) {
			rarity = ChatColor.YELLOW.toString() + ChatColor.ITALIC.toString() + "Unique";
			if (tier == ItemTier.T1) {
				int min_num = 1 + 1;
				int max_num = 2 + 1;
		...
		if (this == HELMET || this == BOOTS) {
			...
			min_armor = (int) (temp_min_armor * 0.50); // 50% of the values of
														// chestplate /
														// leggings.
			max_armor = (int) (temp_max_armor * 0.50); // 50% of the values of
														// chestplate /
														// leggings.

			hp_increase_amount = (int) (temp_hp_increase_amount * 0.50); // 50%
																			// of
																			// HP
																			// increase
																			// value.
		...
		if (regen_type == 0) { // HP/s HP REGEN
			hp_regen = true;
			// hp_regen_max = (int) Math.round((hp_increase_amount / 10) +
			// (hp_increase_amount * 0.05));
			...
			int real_hp_regen_max = (int) ((regen_val) + (hp_increase_amount / 10))
					- r.nextInt((int) Math.round(regen_val));
		...
		if (r.nextInt(100) <= block_chance) {
			block = true;
			block_percent = r.nextInt(block_max - block_min) + ((block_min > 0) ? block_min : 1); // 1
																									// -
																									// 10%
		}
		...
		if (r.nextInt(100) <= dodge_chance) { // 1% chance.
			dodge = true;
			dodge_percent = r.nextInt(dodge_max - dodge_min) + ((dodge_min > 0) ? dodge_min : 1); // 1
																									// -
																									// 3%
		}
		...
		if (r.nextInt(100) <= thorns_chance) { // 1% chance.
			thorns = true;
			thorns_percent = r.nextInt(thorns_max - thorns_min) + ((thorns_min > 0) ? thorns_min : 1); // 1
																										// -
																										// 3%
		}
		...
		if (r.nextInt(100) <= gem_find_chance) { // 1% chance.
			...
			gem_find = true;
			gem_find_percent = r.nextInt(gem_find_max - gem_find_min) + ((gem_find_min > 0) ? gem_find_min : 1); // 1
																													// -
																													// 3%
		}
		...
		if (original != null) {
			...
			if (oarmor_data.contains("hp_regen")) {
				int health_regen_val = Integer.parseInt(oarmor_data.substring(oarmor_data.indexOf("hp_regen=") + 9,
						oarmor_data.indexOf("@hp_regen_split@")));
				hp_regen = true;
				energy_regen = false;
		...
		if (energy_regen == true) {
			...
			// Take from first index of energy_regen= to next index of : from
			// that point.
		}
		...
		/*
		 * if(hp_regen == true){ armor_data += "ls=" + hp_regen_percent + ":";
		 * armor_name = "Lifestealing " + armor_name; armor_description +=
		 * ChatColor.RED.toString() + "LIFE STEAL: " + hp_regen_percent + "%,";
		 * // hp_regen == .split(":")[2] }
		 * 
		 * 
		 * if(block == true){ armor_data += "crit=" + block_percent + ":";
		 * armor_name = "Deadly " + armor_name; armor_description +=
		 * ChatColor.RED.toString() + "CRITICAL HIT: " + block_percent + "%,"; }
		 * 
		 * 
		 * if(hp_increase == true){ armor_data += "edmg=" + element_dmg_type +
		 * "," + element_dmg_amount + ":";
		 * 
		 * if(dodge == true){ armor_name = armor_name + " " +
		 * element_dmg_type.substring(0, 1).toUpperCase() +
		 * element_dmg_type.substring(1, element_dmg_type.length()); } if(dodge
		 * == false){ armor_name = armor_name + " of " +
		 * element_dmg_type.substring(0, 1).toUpperCase() +
		 * element_dmg_type.substring(1, element_dmg_type.length()); }
		 * 
		 * armor_description += ChatColor.RED.toString() +
		 * element_dmg_type.toUpperCase() + " DMG: +" + element_dmg_amount +
		 * ","; // hp_increase == .split(":")[3].split(",")[1] // edmg=fire,3 }
		 */
