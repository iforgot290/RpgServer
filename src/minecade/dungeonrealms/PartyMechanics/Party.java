...
public class Party {
	...
	public void addPlayer(Player pl) {
		...
		for (String s : getPartyMembers()) {
			/*
			 * if(s.equalsIgnoreCase(pl.getName())) { continue; }
			 * if(Bukkit.getPlayer(s) != null) { // Was == continue; }
			 */
			...
			if (party_count == 8) {
				p_mem.sendMessage(ChatColor.GRAY + "You now have " + ChatColor.BOLD + "8/8" + ChatColor.GRAY
						+ " party members. You will now recieve +5% DMG/ARMOR AND " + ChatColor.UNDERLINE + "GREATLY"
						+ ChatColor.GRAY + " increased drop rates when fighting together.");
			}
	...
	public void removePlayer(Player p) {
		...
		/*
		 * for(String jn : getPartyMembers()) { if(getPartyMembers().size() > 1)
		...
		if (p.getName().equalsIgnoreCase(leader.getName()) && getPartyMembers().size() > 0) {
			...
			for (String s : remaining_members) {
				if (Bukkit.getPlayer(s) != null) {
					 ...
					 */
					pty_mem.sendMessage(ChatColor.LIGHT_PURPLE.toString() + "<" + ChatColor.BOLD + "P"
					...
					pty_mem.sendMessage(ChatColor.LIGHT_PURPLE.toString() + "<" + ChatColor.BOLD + "P"
							...
							+ new_leader + ChatColor.GRAY.toString() + " has been promoted to " + ChatColor.UNDERLINE
							+ "Party Leader");
				}
			}
		} else {
	...
	public String getPlayerName(Player p) {
		return ChatColor.WHITE.toString() + (getLeader().getName().equalsIgnoreCase(p.getName())
				? ChatColor.BOLD.toString() + p.getName() : p.getName());
	}
	...
	public void updateScoreboard() {
		...
		while (players.hasNext()) {
			...
			for (String tn : second) {
				/*
				 * if(Bukkit.getPlayer(tn) == null) {
				 * ScoreboardMechanics.getBoard(p).resetScores(Bukkit.
				 * getOfflinePlayer(tn)); continue; }
				 */
				Player t = Bukkit.getPlayer(tn);
