...
public class CommandPPromote implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length != 1) {
			p.sendMessage(
					ChatColor.RED + "" + ChatColor.BOLD + "Invalid Syntax. " + ChatColor.RED + "/ppromote <player>");
			return true;
		}
		...
		if (!(PartyMechanics.isPartyLeader(p.getName()))) {
			if (PartyMechanics.party_map.containsKey(p.getName())) { // In
																		// another
																		// party.
		...
		for (OfflinePlayer mem : p.getScoreboard().getPlayers()) {
			// api.getScoreboard(getPartyTitle(p.getName())).showToPlayer(mem,
			// false);
