...
public class UploadPlayerData extends Thread {
	...
	public void run() {
		...
		try {
			...
			if (!(Hive.no_upload.contains(p_name))) { // Don't upload data if
														// they're in the
														// no_upload list, just
														// set them offline.

				Hive.uploadPlayerDatabaseData(p_name);
				final int n_money = MoneyMechanics.bank_map.get(p_name)
						+ RealmMechanics.getMoneyInInventory(Hive.player_inventory.get(p_name));
				...
				RecordMechanics.updateStatisticData(p_name, n_money, n_pdeaths, n_unlawful_kills, n_lawful_kills,
				...
				if (RealmMechanics.isWorldLoaded(p_name)) {
					RealmMechanics.uploadWorld(p_name, p_name); // mumoxx was
																// here
				}
			...
			if (Hive.server_swap.containsKey(p_name) && Main.plugin.getServer().getPlayer(p_name) != null) {
				...
				// p_name + ":" + Hive.getServerNumFromPrefix(server_prefix),
				// -1, true);
			...
			if (!Hive.player_to_npc.containsKey(p_name) && !(Hive.server_swap.containsKey(p_name))) { // &&
																										// !(HealthMechanics.in_combat.containsKey(p_name))){
			...
			Hive.log.info(Ansi.ansi().fg(Ansi.Color.YELLOW).boldOff().toString()
					+ "[HIVE (SLAVE Edition)] Player data for " + p_name + " uploaded (quit event)."
					+ Ansi.ansi().fg(Ansi.Color.WHITE).boldOff().toString());
		} catch (Exception e) {
			e.printStackTrace();
			if (e instanceof FileNotFoundException) {
				Hive.log.info("[HIVE (SLAVE Edition)] Failed to upload player data for " + p_name
			...
			try {
				...
				if (g_attempts > 5 && !Hive.local_ddos && !Hive.hive_ddos && Hive.isHiveOnline()) {
					Hive.log.info("Failed to upload data for player " + p_name
