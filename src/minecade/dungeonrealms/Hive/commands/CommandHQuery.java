	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (args.length == 3) {
				...
				/*
				 * PreparedStatement pst = null; try{ pst =
				 * ConnectionPool.getConneciton().prepareStatement(
				 * "SELECT pname FROM ban_list WHERE ip IS NOT NULL && pname IS NOT NULL"
				 * );
				 * 
				 * pst.execute(); ResultSet rs = pst.getResultSet();
				 * 
				 * while(rs.next()){ String p_name = rs.getString("pname"); try{
				 * pst = ConnectionPool.getConneciton().prepareStatement(
				 * "DELETE FROM player_database where p_name='" + p_name + "'");
				 * pst.executeUpdate(); pst =
				 * ConnectionPool.getConneciton().prepareStatement(
				 * "DELETE FROM bank_database where p_name='" + p_name + "'");
				 * pst.executeUpdate(); pst =
				 * ConnectionPool.getConneciton().prepareStatement(
				 * "DELETE FROM shop_database where p_name='" + p_name + "'");
				 * pst.executeUpdate(); pst =
				 * ConnectionPool.getConneciton().prepareStatement(
				 * "DELETE FROM statistics where pname='" + p_name + "'");
				 * pst.executeUpdate(); pst =
				 * ConnectionPool.getConneciton().prepareStatement(
				 * "DELETE FROM statistics_old where pname='" + p_name + "'");
				 * pst.executeUpdate(); pst =
				 * ConnectionPool.getConneciton().prepareStatement(
				 * "DELETE FROM perm_statistics where pname='" + p_name + "'");
				 * pst.executeUpdate(); } catch (SQLException ex) {
				 * Hive.log.log(Level.SEVERE, ex.getMessage(), ex);
				 * 
				 * } finally { try { if (pst != null) { pst.close(); }
				 * 
				 * } catch (SQLException ex) { Hive.log.log(Level.WARNING,
				 * ex.getMessage(), ex); } }
				 * 
				 * log.info("Wiped IP banned player " + p_name); }
				 * 
				 * } catch (SQLException ex) { Hive.log.log(Level.SEVERE,
				 * ex.getMessage(), ex);
				 * 
				 * } finally { try { if (pst != null) { pst.close(); }
				 * 
		 ...
		 * 
		 * pst.execute(); ResultSet rs = pst.getResultSet(); while(rs.next()){
		 ...
		 * ex.getMessage(), ex); } }
		 * 
		 * for(String s : reported_offline){ if(!(hasAccountData(s))){ log.info(
		 * "[HIVE] Corrupt player data, " + s); deletePlayerServer(s); }
		 * //setPlayerServer(s, -2); } } });
		 * 
		 * t.start();
		 */
