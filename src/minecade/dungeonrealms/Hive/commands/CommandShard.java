...
public class CommandShard implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (Hive.no_shard) {
			p.sendMessage(ChatColor.RED + "This feature is " + ChatColor.UNDERLINE + "temporarily" + ChatColor.RED
		...
		if (HealthMechanics.in_combat.containsKey(p.getName())) {
			...
			long dif = ((HealthMechanics.HealthRegenCombatDelay * 1000) + HealthMechanics.in_combat.get(p.getName()))
		...
		if ((Hive.seconds_to_reboot <= 10 && Hive.restart_inc) || Hive.server_lock || Hive.local_ddos
				|| Hive.hive_ddos) {
			p.sendMessage(ChatColor.RED + "You " + ChatColor.UNDERLINE + "cannot" + ChatColor.RED
		...
		if (Hive.server_swap.containsKey(p.getName()) || Hive.server_swap_pending.containsKey(p.getName())) {
			p.sendMessage(ChatColor.RED + "You " + ChatColor.UNDERLINE + "cannot" + ChatColor.RED
					+ " change shards while you have another shard request pending.");
			return true;
		}
		...
		if (!p.isOp() && !DuelMechanics.isDamageDisabled(p.getLocation()) && Hive.login_time.containsKey(p.getName())
				&& (((System.currentTimeMillis() - Hive.login_time.get(p.getName())) / 1000.0D) <= 300)) {
			int seconds_left = 300
					- ((int) ((System.currentTimeMillis() - Hive.login_time.get(p.getName())) / 1000.0D));
			p.sendMessage(ChatColor.RED + "You " + ChatColor.UNDERLINE + "cannot" + ChatColor.RED
					+ " change shards while in a wilderness / chaotic zone for another " + ChatColor.UNDERLINE
		...
		Hive.server_swap_pending.put(p.getName(), ""); // Prevent packet abuse,
														// put a map here
														// immediatly.
