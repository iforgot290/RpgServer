	...
	public void run() {
		...
		for (String s : reported_online) {
			if (Hive.pending_upload.contains(s) && sql_reported_online.contains(s)) {
				continue;
			} // They are pending upload and still marked as 'online' this
				// server, so the upload has not finished.
			if (!(actual_online.contains(s)) && !(Hive.lockout_players.contains(s))
					&& !(Hive.player_to_npc.containsKey(s)) && !(Hive.logout_time.containsKey(s))) {
