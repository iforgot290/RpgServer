...
public class CommandResetRealm implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (RealmMechanics.realm_loaded_status.containsKey(p.getName())
		...
		if (RealmMechanics.realm_reset_cd.containsKey(p.getName())) {
			if ((System.currentTimeMillis() - RealmMechanics.realm_reset_cd.get(p.getName())) <= (3600 * 1000)) { // 1
																													// hour.
				p.sendMessage(ChatColor.RED + "You may only reset your realm " + ChatColor.UNDERLINE + "ONCE"
						+ ChatColor.RED + " per hour.");
				return true;
			}
		}
