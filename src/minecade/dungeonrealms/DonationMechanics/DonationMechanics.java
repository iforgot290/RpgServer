...
public class DonationMechanics implements Listener {
	...
	// TODO: Fix sub++ expiring when sub/sub+ expires, prevent sub++ from
	// showing days left in sub.
	...
	public void onEnable() {
		...
		/*
		 * server_list.put(2, "69.69.69.69:6969"); server_list.put(3,
		 * "69.69.69.69:6969"); server_list.put(4, "69.69.69.69:6969");
		 * server_list.put(5, "69.69.69.69:6969"); server_list.put(100,
		 * "69.69.69.69:6969");
		 */

		/*
		 * Do NOT uncomment! The timer is now handled by a separate
		 * donation-dedicated server! if (Bukkit.getMotd().contains("US-0")) {
		 * Calendar nextUpdate =
		 * Calendar.getInstance(TimeZone.getTimeZone("America/New_York"));
		 * nextUpdate.set(nextUpdate.get(Calendar.YEAR),
		 * nextUpdate.get(Calendar.MONTH), nextUpdate.get(Calendar.DATE) + 1, 0,
		 * 0); Calendar now =
		 * Calendar.getInstance(TimeZone.getTimeZone("America/New_York")); Long
		 * msToUp = nextUpdate.getTimeInMillis() - now.getTimeInMillis(); Long
		 * secToUp = TimeUnit.MILLISECONDS.toSeconds(msToUp); log.info(
		 * "[DonationMechanics] Scheduled daily donation duties to run in " +
		 * String.format( "%d hours, %d minutes, %d seconds",
		 * TimeUnit.MILLISECONDS.toHours(msToUp),
		 * TimeUnit.MILLISECONDS.toMinutes(msToUp) -
		 * TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(msToUp)),
		 * secToUp -
		 ...
		 * "[DonationMechanics] Time is 12:00:00 AM ES/DT.  Running daily donation duties."
		 * ); tickSubscriberDays(); log.info(
		 * "[DonationMechanics] Ticked all user's subscriber days forward by ONE."
		 * );
		 * 
		 * tickFreeEcash(); log.info(
		 * "[DonationMechanics] Reset all 'Free E-cash' users, login for more e-cash!"
		 * );
		 * 
		 * tickLifetimeSubEcash(); log.info(
		 * "[DonationMechanics] 999 E-CASH has been given to all sub++ users.");
		 * }
		 * 
	...
	public static void addSubscriberDaysToAll(int number) {
		...
		try {
			...
			pst = con.prepareStatement(
					"UPDATE player_database SET player_database.sdays_left = player_database.sdays_left + ? WHERE player_database.sdays_left IS NOT NULL");
			pst.setInt(1, number);
			pst.executeUpdate();
	...
	public static void addPetToPlayer(UUID id, String pet) {
		...
		try {
			...
			PreparedStatement pst = con.prepareStatement("INSERT INTO player_database (p_name, pets)" + " VALUES" + "('"
					+ id.toString() + "', '" + pet_string + "') ON DUPLICATE KEY UPDATE pets = '" + pet_string + "'");
	 ...
	 * 
	 * @param id
	 *            id of player to add days to
	 * @param days_to_add
	 *            number of days to add
	 * @param set
	 *            true to overwrite, false to add to current days
	 */
	public static void addSubscriberDays(UUID id, int days_to_add, boolean set) {
		...
		try {
			con = ConnectionPool.getConnection();
			pst = con.prepareStatement(
					"INSERT INTO player_database (p_name, sdays_left)" + " VALUES" + "('" + id.toString() + "', '"
							+ days_to_add + "') ON DUPLICATE KEY UPDATE sdays_left ='" + days_to_add + "'");
	 ...
	 * 
	 * @param id
	 *            uuid of the player to download pet data for
	 ...
	 * 
	 * @param player
	 *            player to download pet data for
	 * @return pet data for specified player
	 */
	 ...
	 * 
	 * @param id
	 *            id of player to set rank for
	 * @param rank
	 *            rank to set player to
	 */
	 ...
	 * 
	 * @param id
	 *            uuid of player to get ecash for
	 ...
	 * 
	 * @param id
	 *            id of player to set ecash for
	 * @param amount
	 *            amount of ecash to set
	 */
	public static void setECASH_SQL(UUID id, int amount) {
		...
		try {
			con = ConnectionPool.getConnection();
			pst = con.prepareStatement("INSERT INTO player_database (p_name, ecash)" + " VALUES" + "('" + id.toString()
					+ "', '" + amount + "') ON DUPLICATE KEY UPDATE ecash ='" + amount + "'");
	 ...
	 * server_num, boolean all_servers) { Socket kkSocket = null; PrintWriter
	 * out = null;
	 * 
	 * if(all_servers) { for(int sn : server_list.keySet()) { String ipAndPort =
	 * CommunityMechanics.server_list.get(sn); String ipNoPort =
	 * ipAndPort.contains(":") ?
	 * CommunityMechanics.server_list.get(sn).split(":")[0] : ipAndPort; int
	 * port = ipAndPort.contains(":") &&
	 * StringUtils.isNumeric(ipAndPort.split(":")[1]) ?
	 * Integer.parseInt(ipAndPort .split(":")[1]) : Config.transfer_port; //
	 * Send it anyway, fix for not getting insta-ecash.
	 * //if(server_ip.equalsIgnoreCase(local_ip)){ // continue; // Don't send to
	 * same server. //} try { kkSocket = new Socket(); //kkSocket.bind(new
	 * InetSocketAddress(local_IP, transfer_port+1)); kkSocket.connect(new
	 * InetSocketAddress(ipNoPort, port), 250); out = new
	 * PrintWriter(kkSocket.getOutputStream(), true);
	 * 
	 * out.println(packet_data);
	 * 
	 * } catch(IOException e) { if(out != null) { out.close(); } continue; }
	 * 
	 * if(out != null) { out.close(); } } } else if(!all_servers) { try { String
	 * ipAndPort = CommunityMechanics.server_list.get(server_num); String
	 * ipNoPort = ipAndPort.contains(":") ?
	 * CommunityMechanics.server_list.get(server_num).split(":")[0] : ipAndPort;
	 * int port = ipAndPort.contains(":") &&
	 * StringUtils.isNumeric(ipAndPort.split(":")[1]) ?
	 * Integer.parseInt(ipAndPort .split(":")[1]) : Config.transfer_port;
	 * 
	 * kkSocket = new Socket(); //kkSocket.bind(new InetSocketAddress(local_IP,
	 * transfer_port+1)); kkSocket.connect(new InetSocketAddress(ipNoPort,
	 * port), 250); out = new PrintWriter(kkSocket.getOutputStream(), true);
	 * 
	 * out.println(packet_data);
	 * 
	 * } catch(IOException e) {
