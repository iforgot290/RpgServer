...
public class CommandBossTP implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		// /bosstp X Y Z
		// Checks to make sure enough % of mobs are dead then TP's players to
		// boss room.
		if (args.length != 3) {
			return true;
		}
		...
		// 0 alive and 106 total so (106 - 0) -> 106 / 106 -> 1 * 100 -> 100 -
		// 100
		...
		if (percent_alive >= 25.0D && !instance_name.equalsIgnoreCase("OneWolfeDungeon")) {
			...
			// 150 -> 150 - (Percent alive = 50% -> 25% -> 25% / 100 -> .25 *
			// 150 -> 37.5) -> 150 - 37.5 -> 112.5
			...
			if (mobs_to_kill > 0) {
				for (Player pl : cb.getBlock().getWorld().getPlayers()) {
					pl.sendMessage(ChatColor.RED + "You " + ChatColor.UNDERLINE + "cannot" + ChatColor.RED
							+ " move on to the boss room of this instance until you kill at least " + ChatColor.BOLD
							+ mobs_to_kill + ChatColor.RED + " more monsters.");
				}
				return true;
			}
		}
		...
		if (InstanceMechanics.mob_kill_count.containsKey(cb.getBlock().getWorld().getName())) {
			...
			if ((mobs_killed * 1.90) < total_mobs) {
				// They killed less than 75% of the mobs in the dungeon, yet
				// they're somehow despawned. BUG ABUSEEEE.
				int mobs_to_kill = (int) (total_mobs
						- (Math.round(((mobs_killed * 1.90) / 100.0D) * (double) total_mobs)));
				...
				for (Player pl : cb.getBlock().getWorld().getPlayers()) {
					...
					pl.sendMessage(ChatColor.GRAY
							+ "The system has also detected that some mobs were forcibly despawned rather than slain. If this is the case, you will need to reset the instance.");
				}
				return true;
			}
		}
		...
		for (final Player pl : cb.getBlock().getWorld().getPlayers()) {
			 ...
			 * plugin_instance.getServer().getScheduler().
			 * scheduleSyncDelayedTask(plugin_instance, new Runnable() { public
			 * void run() { setPlayerEnvironment(pl, Environment.NETHER);
			 * pl.setFallDistance(0.0F); } }, 10L); }
			 */
		}
		...
		if (cb.getBlock().getWorld().getName().contains("OneWolfeDungeon")) {
			...
			Location loc = new Location(cb.getBlock().getWorld(), -71, 176, 18); // TODO:
																					// onewolf
																					// -
																					// spawn
																					// his
