	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			p.sendMessage(ChatColor.WHITE
					+ "To designate the entrance to an instance, define a worldguard region and name it prefix it with 'instance_' followed by the name of the instance template. For ex: instance_MapName");
			p.sendMessage(ChatColor.YELLOW
					+ "To designate the exits of an instance, define a worldguard region inside the instance and prefix it with 'exit_instance'. You can have multiple exits, exit_instance1, exit_instance2, etc.");
			p.sendMessage(" ");
			p.sendMessage(ChatColor.YELLOW
					+ "/instance load <instance template> -- Loads a test instance of given template and teleports you there.");
			p.sendMessage(ChatColor.WHITE + "/instance unload <instance> -- Unloads the instance specified.");
			p.sendMessage(ChatColor.YELLOW
					+ "/instance edit <instance template> -- Loads the TEMPLATE of a given instance for editing mobs, loot, or blocks.");
		...
		if (sub_cmd.equalsIgnoreCase("load")) {
			...
			String new_instance = InstanceMechanics.linkInstanceToParty(instance_name, party_members, false); // Define
																												// new
																												// instance.
			if (!(InstanceMechanics.teleport_on_load.contains(p.getName()))) {
				InstanceMechanics.teleport_on_load.add(p.getName());
			}
			InstanceMechanics.syncLoadNewInstance(new_instance, false, false); // Load
																				// new
																				// instance.
		...
		if (sub_cmd.equalsIgnoreCase("edit")) {
			String instance_name = args[1];
			String new_instance = InstanceMechanics.linkInstanceToParty(instance_name,
					PartyMechanics.party_map.get(p.getName()).getPartyMembers(), true); // Define
																						// new
																						// instance.
			if (!(InstanceMechanics.teleport_on_load.contains(p.getName()))) {
				InstanceMechanics.teleport_on_load.add(p.getName());
			}
			InstanceMechanics.syncLoadNewInstance(new_instance, true, false); // Load
																				// new
																				// instance.
