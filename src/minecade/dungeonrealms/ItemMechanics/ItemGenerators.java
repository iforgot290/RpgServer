...
public class ItemGenerators {
	...
	public static ItemStack LeggingsGenerator(int tier, boolean reroll, ItemStack original) {
		...
		/*
		 * int hp_regen_chance = 0; int hp_increase_chance = 0; int
		 * energy_regen_chance = 0;
		 */
		int block_chance = 0;
		int dodge_chance = 0;
		int thorns_chance = 0;
		...
		/*
		 * if(r.nextInt(100) <= energy_regen_chance){ energy_regen = true;
		 * energy_regen_percent = r.nextInt(energy_regen_max - energy_regen_min)
		 * + energy_regen_min; }
		 */

		if (r.nextInt(100) <= block_chance) {
			block = true;
			block_percent = r.nextInt(block_max - block_min) + block_min; // 1 -
																			// 10%
		}

		if (r.nextInt(100) <= dodge_chance) { // 1% chance.
			dodge = true;
			dodge_percent = r.nextInt(dodge_max - dodge_min) + dodge_min; // 1 -
																			// 3%
		}

		if (r.nextInt(100) <= thorns_chance) { // 1% chance.
			thorns = true;
			thorns_percent = r.nextInt(thorns_max - thorns_min) + thorns_min; // 1
																				// -
																				// 3%
		}
		...
		if (r.nextInt(100) <= reflection_chance) { // 1% chance.
			reflection = true;
			try {
				reflection_percent = r.nextInt(reflection_max - reflection_min) + reflection_min; // 1
																									// -
																									// 3%
		...
		if (r.nextInt(100) <= gem_find_chance) { // 1% chance.
			gem_find = true;
			gem_find_percent = r.nextInt(gem_find_max - gem_find_min) + gem_find_min; // 1
																						// -
																						// 3%
		}
		...
		if (r.nextInt(1000) <= item_find_chance) { // 1% chance.
			item_find = true;
			item_find_percent = r.nextInt(item_find_max - item_find_min) + item_find_min; // 1
																							// -
																							// 3%
		}
		 ...
		 * 
		 * armor_description += red.toString() + element_dmg_type.toUpperCase()
		 * + " DMG: +" + element_dmg_amount + ","; // hp_increase ==
	...
	public static ItemStack BootGenerator(int tier, boolean reroll, ItemStack original) {
		...
		Material m = null;
		boolean hp_regen = false, /* hp_increase = false, */energy_regen = false, block = false, dodge = false,
				thorns = false, reflection = false, gem_find = false, item_find = false, speed_boost = false;
		int hp_regen_percent = 0;
		int hp_increase_amount = 0;
		int energy_regen_percent = 0;
		int block_percent = 0;
		...
		hp_increase_amount = (int) (temp_hp_increase_amount * 0.50); // 50% of
																		// HP
																		// increase
																		// value.
	...
	public static ItemStack BowGenorator(int tier, boolean reroll, ItemStack original) {
		Random r = new Random();
		boolean life_steal = false, elemental_dmg = false, slow = false, crit_hit = false,
		...
		int enchant_count = 0;
		// x = durability
		if (reroll == true && original != null) { // Keep the old DMG values
													// instead.
			rarity = ItemMechanics.getItemRarity(original);
			enchant_count = EnchantMechanics.getEnchantCount(original);
		...
		if (vs_modifier_amount > 0) {
			...
			wep_description += red.toString() + "vs. " + vs_modifier_type.toUpperCase() + ": +" + vs_modifier_amount
					+ "% DMG,";
		}
	...
	public static ItemStack AxeGenorator(Material m, boolean reroll, ItemStack original) {
		Random r = new Random();
		boolean life_steal = false, elemental_dmg = false, knockback = false, crit_hit = false, blind = false,
				/* specific_dmg = false, */pure_dmg = false, armor_pen = false;
		// int axe_modifier = 0;
	...
	public static ItemStack SwordGenorator(Material m, boolean reroll, ItemStack original) {
		Random r = new Random();
		boolean life_steal = false, elemental_dmg = false, knockback = false, crit_hit = false, pure_dmg = false,
				armor_pen = false, blind = false/*
												 * , specific_dmg = false
												 */;
		int life_steal_percent = 0;
		int knockback_percent = 0;
