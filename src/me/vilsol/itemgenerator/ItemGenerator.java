...
public class ItemGenerator {
	...
	public ItemGenerator generateItem() {
		...
		if (tier == null)
			tier = ItemTier.values()[r.nextInt(ItemTier.values().length - 1)];
		if (type == null)
			type = ItemType.values()[r.nextInt(ItemType.values().length - 1)];
		if (rarity == null)
			rarity = ItemRarity.values()[r.nextInt(ItemRarity.values().length - 1)];

		ItemStack item = isReroll && origItem != null
				&& (ItemMechanics.isArmor(origItem) || ItemMechanics.isWeapon(origItem)) ? origItem
						: new ItemStack(type.getTier(tier));
		...
		if (!isReroll)
			...
			if (ItemMechanics.isWeapon(origItem))
				meta.setLore(meta.getLore().subList(0, 1)); // strips everything
															// except for dmg
			else if (ItemMechanics.isArmor(origItem))
				meta.setLore(meta.getLore().subList(0, 3)); // strips everything
															// except for
															// dps/armor, hp,
															// and energy/hp
															// regen
		...
		for (ItemModifier modifier : modifierObjects) {
			if (modifier.canApply(type)) {
				if (isReroll && !modifier.isIncludeOnReroll())
					continue;
				...
				if (mc != null) {
					...
					while (bonus != null) {
						...
						if (bonus.getReplacement() != null && bonus.getReplacement().size() > 0) {
							ItemModifier replacement = ItemGenerator.modifiers.get(
									bonus.getReplacement().get(new Random().nextInt(bonus.getReplacement().size())));
		...
		if (isReroll && origItem != null && origItem.hasItemMeta() && origItem.getItemMeta().hasLore()
				&& ItemMechanics.isArmor(origItem)) {
		...
		for (int i = 0; i < bonuses.length; i++) {
			...
			// ARMOR SUFFIXES
			case "ENERGY REGEN":
				name += (name.contains(type.getTierName(tier)) ? " of Fortitude"
						: type.getTierName(tier) + " of Fortitude");
				break;
			case "FIRE RESISTANCE":
				name += (name.contains(type.getTierName(tier)) ? " and Fire Resist"
						: type.getTierName(tier) + " of Fire Resist");
				break;
			case "ICE RESISTANCE":
				name += (name.contains(type.getTierName(tier)) ? " and Ice Resist"
						: type.getTierName(tier) + " of Ice Resist");
				break;
			case "POISON RESISTANCE":
				name += (name.contains(type.getTierName(tier)) ? " and Poison Resist"
						: type.getTierName(tier) + " of Poison Resist");
				break;
			case "GEM FIND":
				name += (name.contains(type.getTierName(tier)) ? " Golden"
						: type.getTierName(tier) + " of Pickpocketing");
				break;
			case "ITEM FIND":
			...
			case "vs. MONSTERS":
				...
				break;
			case "vs. PLAYERS":
				name += (name.contains(type.getTierName(tier)) ? " Slaughter"
						: type.getTierName(tier) + " of Slaughter");
				break;
			case "FIRE DMG":
		...
		if (rarity == ItemRarity.COMMON)
			lore.add(ChatColor.GRAY.toString() + ChatColor.ITALIC.toString() + "Common");
		if (rarity == ItemRarity.UNCOMMON)
			lore.add(ChatColor.GREEN.toString() + ChatColor.ITALIC.toString() + "Uncommon");
		if (rarity == ItemRarity.RARE)
			lore.add(ChatColor.AQUA.toString() + ChatColor.ITALIC.toString() + "Rare");
		if (rarity == ItemRarity.UNIQUE)
			lore.add(ChatColor.YELLOW.toString() + ChatColor.ITALIC.toString() + "Unique");

		if (isReroll && ItemMechanics.isLegacy(origItem))
			lore.add(ChatColor.GOLD.toString() + ChatColor.BOLD + "LEGACY");
		...
		if (isReroll && EnchantMechanics.hasProtection(origItem))
			lore.add(ChatColor.GREEN.toString() + ChatColor.BOLD + "PROTECTED");

		meta.setLore(lore);

		if (!(name.contains(type.getTierName(tier))))
			name += type.getTierName(tier);
		...
		if (isReroll && (ItemMechanics.isECNamed(origItem) || ItemMechanics.isCustomNamed(origItem)))
			meta.setDisplayName(origItem.getItemMeta().getDisplayName());
