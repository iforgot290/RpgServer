...
public abstract class ItemModifier implements Comparable<ItemModifier> {
			...
			if (condition.doesConclude(tier, rarity, meta)) {
				...
				if (condition.getReplacement() != null && condition.getReplacement().size() > 0) {
					ItemModifier replacement = ItemGenerator.modifiers.get(
							condition.getReplacement().get(new Random().nextInt(condition.getReplacement().size())));
	...
	public ItemMeta applyModifier(ModifierCondition condition, ItemMeta meta) {
		...
		random = ((condition.getChosenPrefix() != null) ? condition.getChosenPrefix() : "") + random
				+ ((condition.getChosenSuffix() != null) ? condition.getChosenSuffix() : "");
